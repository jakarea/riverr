	...
	<div class="relative max-w-6xl mx-auto">
		...
		<div class="mt-12 max-w-lg mx-auto grid gap-8 lg:grid-cols-3 lg:max-w-none">
            ...
            @forelse ($articles as $article)
                <div class="flex flex-col rounded-2xl shadow-sm overflow-hidden" style="box-shadow: 0px 7px 22px 0px rgba(143, 134, 195, 0.07)">
                    <a href="{{ url('blog', $article->slug) }}" class="flex-shrink-0 h-64">
                        <img class="h-full w-full object-cover lazy" src="{{ placeholder_img() }}" data-src="{{ src($article->image) }}" alt="{{ $article->title }}">
                    </a>
                    <div class="flex-1 bg-white dark:bg-zinc-800 p-6 flex flex-col justify-between rounded-b-2xl">
                        <div class="flex-1">
                            ...
                            <a href="{{ url('blog', $article->slug) }}" class="block mt-2 truncate overflow-hidden" style="text-overflow: ellipsis;">
                                <p class="text-xl font-semibold text-[#363848] dark:text-gray-200 hover:text-primary-600 truncate">{{ $article->title }}</p>
                            </a>
                        </div>
                    </div>
                </div>
