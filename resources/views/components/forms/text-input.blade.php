...
<div>
    ...
    {{-- Label --}}
    @if ($label)
        <label for="text-input-component-id-{{ $model }}" class="block text-base font-medium {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-[#1F2131] dark:text-white' }}">{{ htmlspecialchars_decode($label) }}</label>
    @endif
    ...
    <div class="{{ $label ? 'mt-2.5' : '' }} relative">
        ...
        <input 
            ...
            class="disabled:cursor-not-allowed focus:!ring-1 block w-full ltr:pr-10 ltr:pl-4 rtl:pl-10 rtl:!pr-4 py-3.5 placeholder:font-normal placeholder:text-[13px] dark:placeholder-zinc-300 text-base bg-[#FBFBFB] font-normal text-[#363848] h-14 dark:text-white rounded-full dark:bg-transparent {{ $errors->first($model) ? 'focus:!ring-red-600 focus:!border-red-600 border-red-500' : 'focus:!ring-primary-600 focus:!border-primary-600 border-[#C2C2C2] dark:border-zinc-500' }}" 
            {{ $attributes }} />
