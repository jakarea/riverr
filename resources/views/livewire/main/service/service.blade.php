        ...
        <div class="lg:grid lg:grid-rows-1 lg:grid-cols-7 lg:gap-x-8 lg:gap-y-10 xl:gap-x-16">
            ...
            <div class="max-w-2xl mx-auto mt-6 md:mt-14 sm:mt-16 lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3">
                ...
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                    ...
                    {{-- Contact seller --}}
                    <a href="{{ url('messages/new', $gig->owner->username) }}" target="_blank" class="w-full bg-primary-100 border border-transparent rounded-full py-4 px-8 flex items-center justify-center text-base font-medium text-primary-600 hover:bg-primary-200 focus:outline-none">{{ __('messages.t_contact_seller') }}</a>
                ...
                <div class="border-t border-gray-200 dark:border-zinc-700 mt-10 pt-10">
                    <h3 class="text-md font-medium text-gray-900 dark:text-gray-300">{{ __('messages.t_actions') }}</h3>
                    <div class="mt-4 text-gray-500">
                    ...
                    <div id="tab-panel-reviews" class="-mb-10" x-data="Components.tabPanel(0)" aria-labelledby="tab-reviews" x-init="init()" x-show="selected" @tab-select.window="onTabSelect" role="tabpanel" tabindex="0">
                        ...
                        <div class="py-6">
                            @foreach ($gig->tags as $tag)
                                @if ($tag)
                                    <a href="{{ url('search?q=' . $tag->name) }}" class="mb-3 text-xs font-semibold inline-block py-2 px-4 rounded-full text-slate-600 dark:text-zinc-400 bg-gray-100 dark:bg-zinc-700 dark:hover:bg-zinc-600 hover:bg-gray-200 last:mr-0 ltr:mr-1 rtl:ml-1">
                                        {{ $tag->name }}
                                    </a>
                                @endif
                            @endforeach
