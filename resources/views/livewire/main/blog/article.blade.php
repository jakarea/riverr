...
<div class="w-full">
    <div class="bg-[#5F4BDB] pt-14 pb-48 text-center">
		<h1 class="text-white text-3xl font-semibold">Blog Details</h1>
...
<div class="max-w-5xl mx-auto -mt-32">
    ...
    {{-- Article content --}}
    <div class="relative py-12 bg-white dark:bg-zinc-800 overflow-hidden rounded-lg shadow ">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8 ql-snow">
            <div class="text-lg w-11/12 mx-auto ql-editor">

                {{-- Title --}}
                <h1 class="mb-6">
                    <span class="block text-sm text-left text-primary-600 font-semibold tracking-wide uppercase">{{ __('messages.t_x_min_read', ['x' => $article->reading_time]) }}</span>
                    <span class="mt-2 block  text-xl text-left leading-8 font-semibold  text-gray-900 dark:text-gray-100 md:text-4xl">{{ $article->title }}</span>
                </h1>
    ...
    @if (settings('blog')->enable_comments)
        <div class="bg-white dark:bg-zinc-800 shadow rounded-lg sm:overflow-hidden mt-10 mb-12">
