    ...
    <div class="container mx-auto px-4">
                            ...
                            @if ($page->column == 1)
                                <li>
                                    @if ($page->is_link && $page->link)
                                        <a href="{{ $page->link }}" target="_blank" class="text-base block font-normal text-white mt-5 hover:ml-1 transition-all dark:text-white">
                                            {{ $page->title }}
                                        </a>
                                    @else
                                        <a href="{{ url('page', $page->slug) }}" class="text-base block font-normal text-white mt-5 hover:ml-1 transition-all dark:text-white">
                                            {{ $page->title }}
                                        </a>
                                    @endif
                                </li>
        ...
        <div class="text-center mt-10">
            ...
            <div class="flex space-x-4 justify-center rtl:space-x-reverse">
                ...
                {{-- Contact --}}
                <a href="{{ url('help/contact') }}" class="text-xs font-medium uppercase text-gray-400 dark:text-gray-400 dark:hover:text-gray-100 hover:underline">{{ __('messages.t_contact_us') }}</a>

                {{-- Privacy policy --}}
                @if (settings('footer')->privacy)
                    <a href="{{ url('page', settings('footer')->privacy->slug) }}" class="text-xs font-medium uppercase text-gray-400 dark:text-gray-400 dark:hover:text-gray-100 hover:underline">{{ __('messages.t_privacy_policy') }}</a>
                @endif

                {{-- Terms --}}
                @if (settings('footer')->terms)
                    <a href="{{ url('page', settings('footer')->terms->slug) }}" class="text-xs font-medium uppercase text-gray-400 dark:text-gray-400 dark:hover:text-gray-100 hover:underline">{{ __('messages.t_terms_of_service') }}</a>
                @endif

                {{-- Blog --}}
                @if (settings('blog')->enable_blog)
                    <a href="{{ url('blog') }}" class="text-xs font-medium uppercase text-gray-400 dark:text-gray-400 dark:hover:text-gray-100 hover:underline">{{ __('messages.t_blog') }}</a>
                @endif
