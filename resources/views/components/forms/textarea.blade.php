...
<div>
    ...
    {{-- Label --}}
    @if ($label)
        <label for="textarea-input-component-id-{{ $model }}" class="block text-base font-medium {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-[#1F2131] dark:text-white' }}">{{ htmlspecialchars_decode($label) }}</label>
    @endif
